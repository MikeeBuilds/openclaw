...
import {
  hasDiscordGuildEntries,
  hasExplicitlyDisabledDiscordChannels,
  resolveDiscordActionGuildEntry,
  resolveDiscordGuildReadDenial,
} from "./runtime.messaging.read-policy.js";
...
export function createDiscordMessagingActionContext(params: {
  ...
  return {
    ...
    assertGuildReadTargetAllowed: async ({
      ...
      const guildInfo = await resolveReadGuildEntry(guildId);
      const denial = resolveDiscordGuildReadDenial({
        directOperator,
        groupPolicy,
        guildInfo,
        filteredChannels: directOperator && filteredResults === true,
      });
      if (denial === "guild") {
        throw new Error("Discord read target channel is not allowed.");
      }
    ...
    resolveDirectoryCurrentChannel: async ({ guilds: candidates }) => {
      if (groupPolicy === "disabled") {
        return undefined;
      }
      ...
      // Guild channel enumeration omits threads. A proven current conversation
      // permits this row, never enumeration of other excluded channels or members.
